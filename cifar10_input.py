...
class AugmentedDataSubset(object):
    ...
    def get_next_batch(self, batch_size, target_class, multiple_passes=False, reshuffle_after_pass=True):
        raw_batch = self.raw_datasubset.get_next_batch(batch_size, multiple_passes,
                                                       reshuffle_after_pass)
        x, y = raw_batch
        mask, xmask = y == target_class, y != target_class
        if mask.sum() < 2 or xmask.sum() < 2:
            raise ValueError('target class empty')
        x_target, y_target = x[mask], y[mask]
        x_others, y_others = x[xmask], y[xmask]
        x_others, y_others = x_others[:x_target.shape[0]], y_others[:x_target.shape[0]]
        # x_others, y_others = x_others[:batch_size//10], y_others[:batch_size//10]
        raw_batch = np.concatenate([x_target, x_others]), np.concatenate([y_target, y_others])
